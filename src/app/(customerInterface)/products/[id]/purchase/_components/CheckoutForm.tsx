...
// layout of the form users can use to make payments
...
// payment form customers can use to make payments
function StripeForm({ price, productId }: { price: number,
    productId: string
}) {
...
async function handleSubmit(e: FormEvent) {
    ...
    if (orderExists) {
        setErrorMessage("You have already purchased this item. Please download it from My Orders Page")
        setIsLoading(false)
        return
    }

    stripe.confirmPayment({ elements, confirmParams: {
        return_url: `${process.env.NEXT_PUBLIC_SERVER_URL}/stripe/purchase_success_page`,
    }}
).then(({ error }) => {
    if (error.type === "card_error" || error.type 
        === "validation_error") {
            ...
            setErrorMessage("Sorry - we are unable to process your payment at this time")
        }  
